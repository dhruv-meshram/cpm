...
void runForwardPassBenchmark() {
    std::string filename = "benchmarks/cpm_engine/results/forward_pass.csv";
    ...
    for (size_t nodes : GRAPH_SIZES) {
        ...
        std::cout << "[ForwardPass] Graph: " << nodes << " nodes / " << edges << " edges" << std::endl;
        ...
        for (int i = 0; i < BENCHMARK_RUNS; ++i) {
            timer.start();
            cpm::ScheduleCalculator::forwardPass(graph, topo_order, project_start);
            timer.stop();
