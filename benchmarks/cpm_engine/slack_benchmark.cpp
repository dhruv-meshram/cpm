...
void runSlackBenchmark() {
    std::string filename = "benchmarks/cpm_engine/results/slack_calculation.csv";
    benchmark::writeCSVRow(filename, {"nodes", "edges", "runtime_ms", "runtime_per_node_ns", "peak_memory_mb"});
    ...
    for (size_t nodes : GRAPH_SIZES) {
        ...
        std::cout << "[Slack] Graph: " << nodes << " nodes / " << edges << " edges" << std::endl;
        ...
        std::cout << "  Peak Memory: " << benchmark::kbToMb(mem.peak_rss_kb) << " MB" << std::endl;
        ...
        benchmark::writeCSVRow(filename, {
            ...
            std::to_string(benchmark::kbToMb(mem.peak_rss_kb))
        });
    }
}
